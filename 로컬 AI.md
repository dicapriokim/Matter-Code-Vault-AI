...
| **가상 환경** | Ubuntu 24.04 Standard LXC | **RAM 8GB+ 권장** |
| **AI 백엔드** | Ollama | 외부 네트워크 오픈 설정 필수 |
| **통신 주소** | `http://127.0.0.1:11434` | 로컬 네트워크 내부 통신 (11434 포트) |
...
| 단계 | 모델명 | 역할 | 상세 임무 |
| :--- | :--- | :--- | :--- |
| **Step 1: Vision** | `moondream:latest` | **눈 (Eye)** | 이미지 분석, Matter QR(MT:) 및 페어링 코드 식별 |
| **Step 2: Reasoning** | `qwen2.5:3b` | **뇌 (Brain)** | 데이터 정제, **오타 교정**, 지능형 기기 작명 |
...
### 🎯 철통 방어 작명 프롬프트 (V3)
`qwen2.5:3b`에게 부여된 오타 교정 및 작명 지침입니다.
...
2. QR 로직 스캔 로직은 핸재 그대로 유지 할건데, AI 부분에 수정이 필요해
 ...
 - 2차 AI 서버(Ollama)는 다음 두 단계를 순차적으로 거쳐야 한다.
   Step 1: Vision Pass (moondream:latest)
   Step 2: Reasoning Pass (qwen2.5:3b)
